...
export async function listVocabulary(page = 1, limit = 40): Promise<ResponseData<WordEntry[]>> {
  ...
  const params = new URLSearchParams({ page: String(page), limit: String(limit) })\n  return request('/?' + params.toString(), 'GET')' + params.toString(), 'GET')
}

/* ¨T¨T¨T Training API ¨T¨T¨T */
...
export async function getQuizWords(bookCode: string, count = 30) {
  const params = new URLSearchParams({ bookCode, count: String(count) })
  return request('/training/quiz/words?' + params.toString(), 'GET')
}
...
/* ¨T¨T¨T Vocabulary Map API ¨T¨T¨T */
import { type VocabularyMapData } from '@/types/map'

export async function loadBookMap(bookCode: string): Promise<ResponseData<VocabularyMapData>> {
  const params = new URLSearchParams()
  return request('/vocab/maps/book/' + bookCode + '?' + params.toString(), 'GET')
}

export async function loadGlobalMap(limit?: number): Promise<ResponseData<VocabularyMapData>> {
  const params = new URLSearchParams({ limit: String(limit ?? 500) })
  return request('/vocab/maps/global?' + params.toString(), 'GET')
}
