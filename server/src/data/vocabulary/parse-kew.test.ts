...
describe('flattenKewBook', () => {
  it('keeps fly as two senses in book 1', () => {
    const result = flattenKewBook(book('kew1', [
  ...
  it('keeps tear as noun and verb in book 2', () => {
    const result = flattenKewBook(book('kew2', [
  ...
  it('splits watch across books by sense key', () => {
    expect(KEW_SENSE_OVERRIDES[kewSenseLookupKey('kew1', 'u7', 'watch')].senseKey).toBe('v')
    expect(KEW_SENSE_OVERRIDES[kewSenseLookupKey('kew3', 'b3', 'watch')].senseKey).toBe('n')
  })
